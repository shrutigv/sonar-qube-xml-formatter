...
var SonarQubeUnitTestReporter = function (baseReporterDecorator, config, logger, helper, formatError) {
  var log = logger.create('reporter.sonarqubeUnitTest')
  var reporterConfig = config.sonarQubeUnitTestReporter || {}
  ...
  this.specSuccess = this.specSkipped = this.specFailure = function (browser, result) {
    ...
    nextPath = filesForDescriptions[preMapped]
    if (filenameFormatter !== null) {
      nextPath = filesForDescriptions[nextPath]
...
SonarQubeUnitTestReporter.$inject = ['baseReporterDecorator', 'config', 'logger', 'helper', 'formatError']

// PUBLISH DI MODULE
module.exports = {
  'reporter:sonarqubeUnitTest': ['type', SonarQubeUnitTestReporter]
}
